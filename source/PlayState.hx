...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'senpai':
				dialogue = CoolUtil.coolTextFile(storyDifficulty > 2 ? 'assets/data/senpai/senpaiDialogue_BSide.txt' : 'assets/data/senpai/senpaiDialogue.txt');
			case 'roses':
				dialogue = CoolUtil.coolTextFile(storyDifficulty > 2 ? 'assets/data/roses/rosesDialogue_BSide.txt' : 'assets/data/roses/rosesDialogue.txt');
			case 'thorns':
				dialogue = CoolUtil.coolTextFile(storyDifficulty > 2 ? 'assets/data/thorns/thornsDialogue_BSide.txt' : 'assets/data/thorns/thornsDialogue.txt');
		}
		...
		else if (SONG.song.toLowerCase() == 'cocoa' || SONG.song.toLowerCase() == 'eggnog')
		{
			...
			upperBoppers.frames = storyDifficulty > 2 ? FlxAtlasFrames.fromSparrow('assets/images/christmas/upperBop_BSIDE.png', 'assets/images/christmas/upperBop_BSIDE.xml') : FlxAtlasFrames.fromSparrow('assets/images/christmas/upperBop.png', 'assets/images/christmas/upperBop.xml');
		...
		{
			curStage = 'mallEvil';
			var bg:FlxSprite = new FlxSprite(-400, -500).loadGraphic(storyDifficulty > 2 ? 'assets/images/christmas/evilBG_BSIDE.png' : 'assets/images/christmas/evilBG.png');
			bg.antialiasing = true;
			bg.scrollFactor.set(0.2, 0.2);
			bg.active = false;
			bg.setGraphicSize(Std.int(bg.width * 0.8));
		...
		var gfVersion:String = storyDifficulty > 2 ? 'gf-bside' : 'gf';

		switch (curStage)
		{
			case 'limo':
				if (storyDifficulty > 2)
					gfVersion = 'gf-car-bside'
				else
					gfVersion = 'gf-car';
			case 'mall' | 'mallEvil':
				if (storyDifficulty > 2)
					gfVersion = 'gf-christmas-bside'
				else
					gfVersion = 'gf-christmas';
			case 'school' | 'schoolEvil':
				if (storyDifficulty > 2)
					gfVersion = 'gf-pixel-bside'
				else
					gfVersion = 'gf-pixel';
		}
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		senpaiEvil.frames = storyDifficulty > 2 ? FlxAtlasFrames.fromSparrow('assets/images/weeb/senpaiCrazy_BSIDE.png', 'assets/images/weeb/senpaiCrazy_BSIDE.xml') : FlxAtlasFrames.fromSparrow('assets/images/weeb/senpaiCrazy.png', 'assets/images/weeb/senpaiCrazy.xml');
	...
	function startSong():Void
	{
		...
		if (!paused)
			if (storyDifficulty > 2)
				FlxG.sound.playMusic("assets/music/" + SONG.song + "_Inst_BSide" + TitleState.soundExt, 1, false);
			else
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (SONG.needsVoices)
			if (storyDifficulty > 2)
				vocals = new FlxSound().loadEmbedded("assets/music/" + curSong + "_Voices_BSide" + TitleState.soundExt);
			else
	...
	private function popUpScore(strumtime:Float):Void
	{
		...
		if (storyDifficulty > 2 && daRating != 'bad')
			bsideShit = "_BSIDE";

		rating.loadGraphic('assets/images/' + pixelShitPart1 + daRating + pixelShitPart2 + bsideShit + ".png");
		rating.screenCenter();
